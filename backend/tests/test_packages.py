...
from check_packages import check_exact, check_manifests, check_side, pep503
...
def test_specifier_mismatch_fails():
    fails = check_side("backend", {"flask": "==99.0.0"}, {"flask": "==3.0.0"})
    assert any("does not match pin" in line for line in fails)


def test_range_and_caret_fail():
    ranges = check_exact("backend", {"flask": ">=3.0.0"}, "python")
    carets = check_exact("frontend", {"react": "^18.3.1"}, "node")
    tildes = check_exact("frontend", {"vite": "~5.4.8"}, "node")
    assert ranges
    assert carets
    assert tildes
    assert any("is not an exact pin" in line for line in ranges + carets + tildes)
